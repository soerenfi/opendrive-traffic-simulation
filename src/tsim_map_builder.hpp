...
public:
  ...
  std::shared_ptr<JunctionConnection> junction_addConnection(Junction* junction, uint32_t incoming_road,
  ...
  std::shared_ptr<Lane> laneSection_addLane(std::shared_ptr<LaneSection> lane_section, uint32_t id, double offset,
                                            double width, LaneType type);
